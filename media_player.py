...
# Default folder used for local media files
DEFAULT_LOCALTRACK_FOLDER = Path(__file__).parent / "localtracks"
DEFAULT_LOCALTRACK_FOLDER.mkdir(exist_ok=True)

# Patterns for parsing URLs and timestamps
...
class AsyncIter:
    """Minimal async iterator wrapper."""
...
class LocalPath:
    """Handle local media files in a cross-platform manner."""

    _all_music_ext = _FULLY_SUPPORTED_MUSIC_EXT + _PARTIALLY_SUPPORTED_MUSIC_EXT

    def __init__(self, path, localtrack_folder=DEFAULT_LOCALTRACK_FOLDER, **kwargs):
        self._localtrack_folder = Path(localtrack_folder)
        self._path = path
        if isinstance(path, (Path, LocalPath)):
            path = str(Path(path).absolute())
        elif path is not None:
            path = str(path)

        self.cwd = Path.cwd()
        _lt_folder = self._localtrack_folder
        _path = Path(path) if path else self.cwd
        if kwargs.get("forced"):
            if _path.parts and _path.parts[-1].lower() == "localtracks":
                self.localtrack_folder = _path
            else:
                self.localtrack_folder = _path / "localtracks"
        else:
            if _lt_folder.parts and _lt_folder.parts[-1].lower() == "localtracks":
    ...
    def rglob(self, pattern, folder=False) -> Iterator[str]:
        if folder:
            return glob.iglob(f"{glob.escape(str(self.path))}{os.sep}**{os.sep}", recursive=True)
        else:
            return glob.iglob(
                f"{glob.escape(str(self.path))}{os.sep}**{os.sep}*{pattern}", recursive=True
            )

    def glob(self, pattern, folder=False) -> Iterator[str]:
        if folder:
            return glob.iglob(f"{glob.escape(str(self.path))}{os.sep}*{os.sep}", recursive=False)
        else:
            return glob.iglob(f"{glob.escape(str(self.path))}{os.sep}*{pattern}", recursive=False)
    ...
    async def tracks_in_tree(self):
        ...
        async for track in self.multirglob(*[f"{ext}" for ext in self._all_music_ext]):
            with contextlib.suppress(ValueError):
                if track.path.parent != self.localtrack_folder and track.path.relative_to(self.path):
    ...
    def __eq__(self, other):
        if isinstance(other, LocalPath):
            ...
            return self.path.parts == other.parts
    ...
    def __hash__(self):
        try:
            ...
            self._hash = hash(tuple(self.path.parts))
            return self._hash
...
class Query:
    """Parsed query information for the media player."""
