...
class LocalPath:
    ...
    def rglob(self, pattern, folder=False) -> Iterator[str]:
        if folder:
            return glob.iglob(
                f"{glob.escape(str(self.path))}{os.sep}**{os.sep}", recursive=True
            )
        else:
            return glob.iglob(
                f"{glob.escape(str(self.path))}{os.sep}**{os.sep}*{pattern}",
                recursive=True,
            )
    ...
    def glob(self, pattern, folder=False) -> Iterator[str]:
        if folder:
            return glob.iglob(
                f"{glob.escape(str(self.path))}{os.sep}*{os.sep}", recursive=False
            )
        else:
            return glob.iglob(
                f"{glob.escape(str(self.path))}{os.sep}*{pattern}", recursive=False
            )
